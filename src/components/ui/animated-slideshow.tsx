...
>(({ children, className, ...props }, ref) => {
  ...
  return (
    <HoverSliderContext.Provider value={{ activeSlide, changeSlide }}>
      <div className={className} ref={ref as React.ForwardedRef<HTMLDivElement>} {...props}>
        {children}
      </div>
    ...
    >
      {characters.map((char, charIndex) => (
        ...
        >
          ...
          >
            <motion.span
              className={cn("inline-block", isActive ? "opacity-0" : "opacity-40")}
              initial={{ y: "0%" }}
              animate={isActive ? { y: "-150%" } : { y: "0%" }}
            >
              {char === " " ? "\u00A0" : char}
            </motion.span>
